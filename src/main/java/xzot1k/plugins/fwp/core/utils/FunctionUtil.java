...
public class FunctionUtil {
    ...
    public void performLightningWandMagic(Player player, Creeper creeper) {
        ...
        if (pluginInstance.getManager().doesWPTypeMatchItem(WPType.LIGHTNING_WAND, handItemStack)) {
            ...
            boolean useGlobalCooldown = pluginInstance.getConfig().getBoolean("lightning-wand-section" + ".use-global-cooldown");
            ...
            WPData wpData = new WPData(player, handItemStack);
            wpData.removeUses(player, handItemStack, 1);
            wpData.apply(player, handItemStack);

            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.LIGHTNING_WAND);
    ...
    public void performSandWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        else cooldownRemainder = pluginInstance.getManager().getCooldownRemainder(player, WPType.SAND_WAND, cooldown);
        ...
        if ((onlyRemoveSand && clickedBlock.getType() == Material.SAND) || (!onlyRemoveSand && pluginInstance.getManager().isInList(clickedBlock.getType(),
            ...
            clickedBlock.getWorld().playSound(clickedBlock.getLocation(), (pluginInstance.getServerVersion().startsWith("v1_8")
        ...
        if (cooldown > -1) {
            if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.SAND_WAND);
            else pluginInstance.getManager().updateGlobalCooldown(player);
        }
    }
    ...
    public void performCraftWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        if (pluginInstance.getManager().isInList(clickedBlock.getType(), clickedBlock.getData(), "craft-wand-section.container-materials",
                pluginInstance.getConfig().getBoolean("general-section.blacklist-effected-materials"), pluginInstance.getConfig().getBoolean("general-section" +
                        ".loose-check-effected-materials"))
            ...
            for (int i = -1; ++i < countingMaterials.size(); ) {
                ...
                String solutionMaterialString = pluginInstance.getManager().getCraftSolution(material,
                        (short) ((material.name().equalsIgnoreCase("INK_SACK") || material.name().equalsIgnoreCase("INK_SAC")) ? 4 : 0));
                int inventoryStock = materialCountingMap.get(material), craftRequirement = pluginInstance.getManager().getCraftAmount(material,
                        (short) ((material.name().equalsIgnoreCase("INK_SACK")
                                || material.name().equalsIgnoreCase("INK_SAC")) ? 4 : 0)), blocksToAdd = (inventoryStock / craftRequirement), amountToRemove =
                        (craftRequirement * (inventoryStock / craftRequirement));
            ...
            } else pluginInstance.getManager().sendCustomMessage(player, "not-enough-crafted-message");

            if (cooldown > -1)
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.CRAFT_WAND);
                else pluginInstance.getManager().updateGlobalCooldown(player);
        }
    }
    ...
    public void performSellWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        long cooldownRemainder;
        if (useGlobalCooldown) cooldownRemainder = pluginInstance.getManager().getGlobalCooldownRemainder(player, cooldown);
        else cooldownRemainder = pluginInstance.getManager().getCooldownRemainder(player, WPType.SELL_WAND, cooldown);
        ...
        if (pluginInstance.getManager().isInList(clickedBlock.getType(), clickedBlock.getData(), "sell-wand-section.container-materials",
            ...
            if (totalEarnings > 0) {
                ...
                WPData wpData = new WPData(player, handItemStack);
                wpData.setSoldItems(wpData.getSoldItems() + totalItems);
                wpData.removeUses(player, handItemStack, 1);
                wpData.apply(player, handItemStack);
            ...
            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.SELL_WAND);
                else pluginInstance.getManager().updateGlobalCooldown(player);
            }
        }
    }
    ...
    public void performSmeltWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        if (pluginInstance.getManager().isInList(clickedBlock.getType(), clickedBlock.getData(), "smelt-wand-section.container-materials",
            ...
            for (int i = -1; ++i < inventoryHolder.getInventory().getSize(); ) {
                ...
                if ((itemStack != null && itemStack.getType() != Material.AIR)) {
                    if (!materialCountingMap.isEmpty() && materialCountingMap.containsKey(itemStack.getType() + ":" + itemStack.getDurability()))
                        materialCountingMap.put(itemStack.getType() + ":" + itemStack.getDurability(),
                                materialCountingMap.get(itemStack.getType() + ":" + itemStack.getDurability()) + itemStack.getAmount());
                    else
            ...
            if (!(totalSmelted <= 0)) {
                ...
                pluginInstance.getManager().sendCustomMessage(player, "smelted-message",
                        "{total-smelted}:" + totalSmelted, "{total-consumed}:" + totalConsumed);
            } else pluginInstance.getManager().sendCustomMessage(player, "not-enough-smelted-message");

            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.SMELT_WAND);
                else pluginInstance.getManager().updateGlobalCooldown(player);
            }
        }
    ...
    public void performTNTWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        if (pluginInstance.getManager().isInList(clickedBlock.getType(), clickedBlock.getData(), "tnt-wand-section.container-materials",
            ...
            if (totalFoundTnt > 0) {
                ...
                pluginInstance.getManager().sendCustomMessage(player, "tnt-wand-message", ("{total}:" + totalFoundTnt));
    ...
    public void performIceWandMagic(Player player, Block clickedBlock, ItemStack handItemStack) {
        ...
        if (cooldown > -1) {
            if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.ICE_WAND);
            else pluginInstance.getManager().updateGlobalCooldown(player);
        }
    }
    ...
    public void performProjectileWandMagic(Player player, ItemStack handItemStack) {
        ...
        if (cooldown > -1) {
            if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.PROJECTILE_WAND);
            else pluginInstance.getManager().updateGlobalCooldown(player);
        }
    }
    ...
    public void performWallWandMagic(Player player, ItemStack handItemStack) {
        ...
        for (int i = -1; ++i < itemRadius; )
            for (int j = -1; ++j < wallLength; ) {
                ...
                Block currentBlockNegative = (blockFace == BlockFace.EAST || blockFace == BlockFace.WEST)
                        ? centerBlock.getRelative(0, i, -j) : centerBlock.getRelative(-j, i, 0);

                if (material != null && pluginInstance.getManager().isLocationSafe(currentBlockNegative.getLocation(), player, false, true, material)
                        && pluginInstance.getManager().isInList(currentBlockNegative.getType(), currentBlockNegative.getData(),
                        "wall-wand-section.replaceable-materials", blacklist, looseCheck)) {
                    pluginInstance.getManager().logToCoreProtect(player, currentBlockNegative, true, false);
                    currentBlockNegative.setType(material);

                    if (!pluginInstance.getServerVersion().startsWith("v1_13") && !pluginInstance.getServerVersion().startsWith("v1_14")
        ...
        centerBlock.getWorld().playSound(centerBlock.getLocation(), (pluginInstance.getServerVersion().startsWith("v1_7")
                || pluginInstance.getServerVersion().startsWith("v1_8")) ? Sound.valueOf("STEP_STONE") : Sound.BLOCK_STONE_BREAK, 1, 1);
        ...
        if (cooldown > -1) {
            if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.WALL_WAND);
            else pluginInstance.getManager().updateGlobalCooldown(player);
        }
    }
    ...
    public void performPlatformWandMagic(Player player, ItemStack handItemStack) {
        ...
        long cooldownRemainder;
        if (useGlobalCooldown) cooldownRemainder = pluginInstance.getManager().getGlobalCooldownRemainder(player, cooldown);
        else cooldownRemainder = pluginInstance.getManager().getCooldownRemainder(player, WPType.PLATFORM_WAND, cooldown);
        if (!(cooldown <= -1) && cooldownRemainder > 0) {
            pluginInstance.getManager().sendCustomMessage(player, "on-cooldown-message", "{time-left}:" + cooldownRemainder);
            return;
        }

        Material material = Material.getMaterial(Objects.requireNonNull(pluginInstance.getConfig().getString("platform-wand-section.build-material"))
                .toUpperCase().replace(" ", "_").replace("-", "_"));
    ...
    public void performHarvesterAxeMagic(@NotNull Player player, Block block, ItemStack handItemStack) {
        ...
        long cooldownRemainder;
        if (useGlobalCooldown) cooldownRemainder = pluginInstance.getManager().getGlobalCooldownRemainder(player, cooldown);
        else cooldownRemainder = pluginInstance.getManager().getCooldownRemainder(player, WPType.HARVESTER_AXE, cooldown);
        ...
        if (pluginInstance.getManager().isHAMaterial(block)) {
            ...
            Material melonMaterial = Material.getMaterial((pluginInstance.getServerVersion().startsWith("v1_7") || pluginInstance.getServerVersion().startsWith("v1_8")
                    || pluginInstance.getServerVersion().startsWith("v1_9") || pluginInstance.getServerVersion().startsWith("v1_10")
                    || pluginInstance.getServerVersion().startsWith("v1_11") || pluginInstance.getServerVersion().startsWith("v1_12")) ? "MELON_BLOCK" : "MELON");
            if (inSellMode) {
                if (block.getType() == melonMaterial && handItemStack.getEnchantments().containsKey(Enchantment.SILK_TOUCH)) {
                    ...
                    for (ItemStack dropItem : block.getDrops(handItemStack)) {
                        ...
                        if (pluginInstance.getShopGUIPlusHandler() != null) itemPrice = pluginInstance.getShopGUIPlusHandler().getItemStackPriceSell(player, duplicate);
                        else if (pluginInstance.isEssentialsInstalled()) itemPrice = pluginInstance.getManager().getEssentialsSellPrice(duplicate);
                        else itemPrice = pluginInstance.getManager().getMaterialPrice(duplicate, "harvester-axe-section.material-prices");
            ...
            if (inSellMode) {
                if (!pluginInstance.getConfig().getBoolean("harvester-axe-section.custom-item-currency")) {
                    if (pluginInstance.getConfig().getBoolean("general-section.use-vault")) {
                        ...
                        pluginInstance.getServer().getPluginManager().callEvent(transactionEvent);
                        if (!transactionEvent.isCancelled()) pluginInstance.getVaultHandler().getEconomy().depositPlayer(player, totalEarnings);
                    } else {
                        pluginInstance.getManager().sendCustomMessage(player, "vault-disabled");
                        return;
                    }
                } else {
                    if (player.getInventory().firstEmpty() == -1) player.getWorld().dropItemNaturally(player.getLocation(),
                            pluginInstance.getManager().getCustomItem((int) totalEarnings, "harvester-axe-section.custom-item"));
                    else player.getInventory().addItem(pluginInstance.getManager().getCustomItem((int) totalEarnings, "harvester-axe-section.custom-item"));
                }

                wpData.setSoldItems(wpData.getSoldItems() + totalItems);
                pluginInstance.getManager().sendCustomMessage(player, "sold-message", "{total-earnings}:" + totalEarnings, "{total-items}:" + totalItems);
            } else pluginInstance.getManager().sendCustomMessage(player, "added-message", "{total-earnings}:" + totalEarnings, "{total-items}:" + totalItems);

            wpData.apply(player, handItemStack);

            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.HARVESTER_AXE);
                else pluginInstance.getManager().updateGlobalCooldown(player);
            }
        }
    }
    ...
    public void performTrayPickaxeMagic(BlockBreakEvent event, Player player, Block block, ItemStack handItemStack) {
        ...
        if (pluginInstance.getManager().isInList(block.getType(), block.getData(), "tray-pickaxe-section.trigger-materials",
            ...
            WPData wpData = new WPData(player, handItemStack);
            wpData.removeUses(player, handItemStack, 1);
            wpData.setBlocksBroken(wpData.getBlocksBroken() + 1);
            ...
            if (blockCounter > 0) {
                wpData.setBlocksBroken(wpData.getBlocksBroken() + blockCounter);
            ...
            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.TRAY_PICKAXE);
                else pluginInstance.getManager().updateGlobalCooldown(player);
            }
        }
    }
    ...
    public void performTrenchPickaxeMagic(Player player, Block block, ItemStack handItemStack) {
        ...
        if (!pluginInstance.getManager().isInList(block.getType(), block.getData(), "trench-pickaxe-section.non-effected-materials",
            ...
            block.getWorld().playSound(block.getLocation(), (pluginInstance.getServerVersion().startsWith("v1_8")
                    || pluginInstance.getServerVersion().startsWith("v1_7")) ? Sound.valueOf("STEP_STONE") : Sound.BLOCK_STONE_BREAK, 1, 1);
            ...
            if (blockCounter > 0) wpData.setBlocksBroken(wpData.getBlocksBroken() + (blockCounter + 1));

            wpData.apply(player, handItemStack);

            if (cooldown > -1) {
                if (!useGlobalCooldown) pluginInstance.getManager().updateWPCooldown(player, WPType.TRENCH_PICKAXE);
                else pluginInstance.getManager().updateGlobalCooldown(player);
            }
        }
    }
    ...
    public void performSpawnerPickaxeMagic(Player player, Block block, ItemStack handItemStack) {
        ...
        boolean isPlaced = isPlaced(block);
